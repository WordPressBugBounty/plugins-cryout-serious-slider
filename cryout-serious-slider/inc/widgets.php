...
class Cryout_Serious_Slider_Widget extends WP_Widget { 	
	...
	function form($instance) {
		...
		?>
		<div>
			<p><label for="<?php echo esc_attr($this->get_field_id('sid')); ?>"><?php esc_html_e('Displayed Slider', 'cryout-serious-slider') ?>:</label> 
				<select class="widefat" id="<?php echo esc_attr($this->get_field_id('sid')); ?>" name="<?php echo esc_attr($this->get_field_name('sid')); ?>">
					<?php foreach ($sliders as $slider) { ?>
						<option value="<?php echo esc_attr($slider['value']) ?>" <?php selected($slider['value'],$sid) ?>><?php echo wp_kses_post($slider['text']) ?></option>
					<?php } ?> 
				</select> 
			</p>
		</div> <?php  
